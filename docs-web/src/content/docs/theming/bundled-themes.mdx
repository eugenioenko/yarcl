...
| **Brutalist** | `brutalist` | yellow on black and white | square corners, 2px borders, monospace | hard offset shadows, dashed focus ring |
| **Soft** | `soft` | violet on lavender | pill controls, round cards, rounded sans | large 44px controls, airy spacing, soft tinted shadows |
| **Compact** | `compact` | teal on cool gray | 2px corners, 13px sans | 28px controls, tight spacing, compact tables, nearly flat |
| **Editorial** | `editorial` | terracotta on cream | square corners, serif throughout | generous spacing, flat cards, double focus ring |
...
export { soft as default } from 'yarcl/themes';
```
...
export default defineConfig({
  ...brutalist,
  colors: { ...brutalist.colors, primary: { light: '#00e5ff', dark: '#00e5ff' } },
});
```
...
Themes name their font families (JetBrains Mono, Nunito, IBM Plex Sans, Iowan Old Style) but don't ship font files. Without them, each stack falls back to a similar system font. To load them, add `typography.fontFaces` with your own files, or load them from a font service in your HTML.
