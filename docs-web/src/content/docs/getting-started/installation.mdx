...
yarcl needs **React 19 or newer** and **Vite 6.1 or newer**.

## Automatic setup

Run the setup command from an existing Vite React project:

<Tabs>
  <TabItem label="pnpm">
  ```sh
  pnpm dlx @yarcl/react init
  ```
  </TabItem>
  <TabItem label="npm">
  ```sh
  ...
  <TabItem label="yarn">
  ```sh
  yarn dlx @yarcl/react init
  ```
  </TabItem>
</Tabs>

This installs `@yarcl/react`, adds the Vite plugin, updates `tsconfig.app.json` or `tsconfig.json`, and creates `src/yarcl.config.ts`. Existing JSON comments and unrelated settings are preserved. Run the command again to verify or repair the setup.

Use `--config <path>` to choose a different config file or `--skip-install` to update the project without running its package manager.
...
The Vite plugin and the TypeScript `paths` entry must point at the same file. The plugin stops with a clear error if they differ. Re-run the setup command to repair the mapping, or see [the Vite plugin](/configuration/vite-plugin/) for details.
